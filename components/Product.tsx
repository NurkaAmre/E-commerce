...
export default function Product({ product }: { product: ProductType }) {
  return (
    <Link href={`/product/${product.slug.current}`} key={product.id}>
      <div className="flex flex-col px-4 h-[270px] w-[280px] md:w-[375px] md:h-[375px] rounded-lg justify-center items-center -inset-2 shadow-2xl">
        <Image
        ...
        <div className="flex justify-between items-center my-4 w-full max-w-[400px]">
          <div className="flex text-lg gap-4 text-gray-600 font-[dancingScript]">
            <h3 >{product.name}</h3>
            <h2>{product.price}&#x20B8;</h2>
          </div>
